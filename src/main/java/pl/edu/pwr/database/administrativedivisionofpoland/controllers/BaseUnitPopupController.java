...
public abstract class BaseUnitPopupController {
    ...
    public void initializeTabPaneListeners(){
        addressSelectionTabPane.getSelectionModel().selectedItemProperty().addListener((observable, oldValue, newValue) -> {
            ...
            if(Objects.equals(newValue.getId(), "1")){
                ...
                try {
                    PageResult<OfficeAddressDto> requestResult = resultFetcher.getAddresses(1, Integer.MAX_VALUE);
                    Platform.runLater(() -> {
            ...
            if(Objects.equals(newValue.getId(),"2")){
                ...
                postalCodeTextField.textProperty().addListener(new ChangeListener<>() {
                    @Override
                    public void changed(ObservableValue<? extends String> observableValue, String oldValue, String newValue) {
                        confirmButton.setDisable(!newValue.matches("[0-9]{2}-[0-9]{3}"));
                    }
                });
            }
        });
    ...
    public void onCloseButtonClick(ActionEvent ignoredActionEvent) {
